...
export function PersonaCard({ persona = "Exit Liquidity Human", description = "You traded 847 times last quarter...", stats = {} }: { persona?: string, description?: string, stats?: Record<string, string | number> }) {
  return (
    <div className="relative overflow-hidden bg-linear-to-br from-purple-900 to-slate-900 border border-purple-500/30 rounded-3xl p-8 shadow-[0_0_40px_rgba(168,85,247,0.2)] aspect-4/5 flex flex-col justify-between max-w-sm mx-auto w-full group">
      ...
      <div className="relative z-10">
        ...
        <h2 className="text-4xl font-black text-white leading-tight">You are:<br/><span className="text-transparent bg-clip-text bg-linear-to-r from-purple-400 to-pink-500">{persona}</span></h2>
      ...
      <div className="relative z-10 space-y-4 font-mono">
        <div className="bg-black/40 backdrop-blur border border-purple-500/20 p-4 rounded-xl">
          ...
          <div className="text-xl text-white font-bold">{stats.winRate || "12.4%"}</div>
        ...
        <div className="bg-black/40 backdrop-blur border border-purple-500/20 p-4 rounded-xl flex justify-between items-center">
          <div>
            ...
            <div className="text-lg text-white font-bold">{stats.topTradedToken || "$WIF"}</div>
          </div>
          <div className="text-right">
            <div className="text-xs text-purple-400 mb-1">Total PnL</div>
            <div className="text-lg text-red-400 font-bold">{stats.totalPnl || "-$4,250"}</div>
          </div>
        </div>
      </div>
